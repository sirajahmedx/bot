...
class GitHubAutoFollow {
   ...
   */
  async initialize(username, mode, user = "user1") {
    try {
      // Load environment variables for the specified user
      const envFile = user ? `.env.${user}` : ".env";
      ...
      if (!token) {
        throw new Error(
          `GITHUB_TOKEN environment variable is required. Please check your ${envFile} file.`
        );
      }

      logger.info(`🔑 Using token from ${envFile}`);
      ...
      logger.info(
        `Target user: ${targetUser.login} (${targetUser.followers} followers, ${targetUser.following} following)`
      );
      ...
      if (apiStatus) {
        logger.info(
          `API Rate Limit: ${apiStatus.remaining}/${
            apiStatus.limit
          } remaining, resets at ${apiStatus.reset.toLocaleTimeString()}`
        );
      }

      // Initialize or load progress with user-specific file
      const sessionLimit = generateSessionLimit(this.config);
      const progressFile = user ? `progress.${user}.json` : "progress.json";
      ...
      logger.info(`📁 Using progress file: ${progressFile}`);
  ...
  async processPage(users) {
    ...
    const newUsers = users.filter(
      (user) => !isUserFollowed(this.progress, user.login)
    );
    ...
    logger.info(
      `Page ${this.progress.currentPage}: Following ${
        selectedUsers.length
      } out of ${newUsers.length} new users (${Math.round(
        (selectedUsers.length / newUsers.length) * 100
      )}%)`
    );
   ...
   * @param {string} user - User number (user1, user2, etc.)
   */
  async run(username, mode, user = "user1") {
    try {
      this.isRunning = true;
      this.shouldStop = false;
      ...
      while (hasMorePages && !this.shouldStop) {
        try {
          ...
          // Small delay between pages with progressive increase
          const pageDelay = 2000 + Math.floor(currentPage / 5) * 1000; // Increase delay every 5 pages
          logger.info(`Waiting ${pageDelay}ms before next page...`);
      ...
      logger.info(
        `  Users followed: ${finalStats.followed}/${finalStats.limit}`
      );
      logger.info(`  Pages processed: ${pagesProcessed}`);
      logger.info(
        `  Duration: ${Math.round(finalStats.duration / 60)} minutes`
      );
      logger.info(
        `  Status: ${finalStats.isComplete ? "Complete" : "Incomplete"}`
      );
    } catch (error) {
...
async function main() {
  ...
  program
    ...
    .description(
      "Automated GitHub following tool with intelligent rate limiting"
    )
    .version("1.0.0")
    ...
    .option(
      "--user <user>",
      "User account (user1, user2, user3, user4)",
      "user1"
    )
    .action(async (options) => {
      ...
      try {
        ...
        logger.info(`Using account: ${options.user}`);
        ...
        await app.run(options.username, options.mode, options.user);
