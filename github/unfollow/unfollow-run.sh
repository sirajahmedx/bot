...
# Usage: ./unfollow-run.sh <user> <target_username> <mode>
# Example: ./unfollow-run.sh user1 phntmzn followers

if [ $# -ne 3 ]; then
    echo "Usage: ./unfollow-run.sh <user> <target_username> <mode>"
    echo "Users: user1, user2, user3, user4"
    echo "Modes: followers, following"
    echo ""
    echo "Examples:"
    echo "  ./unfollow-run.sh user1 phntmzn followers"
    echo "  ./unfollow-run.sh user2 octocat following"
    exit 1
fi
