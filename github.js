...
async function checkRateLimit() {
  if (rateLimitInfo.remaining <= 10) {
    ...
    if (waitTime > 0) {
      logger.warn(`Rate limit nearly exhausted (${rateLimitInfo.remaining} remaining). Waiting until ${rateLimitInfo.reset.toLocaleTimeString()}`);
  ...
  for (let attempt = 0; attempt < config.retryAttempts; attempt++) {
    try {
      ...
      if (attempt < config.retryAttempts - 1) {
        const delay = getRetryDelay(config, attempt);
        logger.warn(`${operation} failed (attempt ${attempt + 1}/${config.retryAttempts}): ${error.message}. Retrying in ${delay}ms`);
        await new Promise(resolve => setTimeout(resolve, delay));
      }
    }
  }
 ...
 */
export async function fetchUsers(username, mode, page, perPage, config) {
  if (!octokit) {
    throw new Error('GitHub client not initialized');
  }
  ...
  try {
    ...
    logger.debug(`Fetched ${users.length} users from ${username}/${mode} (page ${page})`);
    return users;
  } catch (error) {
    if (error.status === 404) {
      throw new Error(`User '${username}' not found or their ${mode} list is private`);
    }
    if (error.status === 401 || error.status === 403) {
      throw new Error(`Authentication failed or insufficient permissions: ${error.message}`);
    }
...
export async function followUser(username, config) {
  ...
  try {
    ...
    if (error.status === 401 || error.status === 403) {
      throw new Error(`Authentication failed or insufficient permissions to follow users: ${error.message}`);
    }
...
export async function isFollowing(username, config) {
  ...
  try {
    await executeWithRetry(
      () => octokit.rest.users.checkPersonFollowingForUser({ username }),
      config,
      operation
    );
    ...
    logger.warn(`Failed to check following status for ${username}: ${error.message}`);
...
export async function validateUser(username, config) {
  ...
  try {
    ...
    logger.debug(`User ${username} validated: ${user.name || 'No name'} (${user.public_repos} repos, ${user.followers} followers)`);
