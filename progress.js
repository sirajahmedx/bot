...
export function loadProgress() {
  try {
    ...
    const validatedProgress = {
      ...DEFAULT_PROGRESS,
      ...progress,
      followedUsers: Array.isArray(progress.followedUsers) ? progress.followedUsers : [],
      currentPage: typeof progress.currentPage === 'number' ? progress.currentPage : 1,
      totalFollowed: typeof progress.totalFollowed === 'number' ? progress.totalFollowed : 0,
      sessionLimit: typeof progress.sessionLimit === 'number' ? progress.sessionLimit : 0
    };

    logger.debug(`Progress loaded: ${validatedProgress.totalFollowed} users followed, page ${validatedProgress.currentPage}`);
...
export function saveProgress(progress) {
  try {
    ...
    logger.debug(`Progress saved: ${progress.totalFollowed} users followed, page ${progress.currentPage}`);
  } catch (error) {
 ...
 */
export function initializeProgress(targetUsername, mode, sessionLimit) {
  const existingProgress = loadProgress();
  
  // Check if this is a continuation of the same session
  const isSameSession = existingProgress.targetUsername === targetUsername && 
                       existingProgress.mode === mode &&
                       existingProgress.totalFollowed < existingProgress.sessionLimit;
  
  if (isSameSession) {
    logger.info(`Resuming previous session: ${existingProgress.totalFollowed}/${existingProgress.sessionLimit} follows completed`);
...
export function getSessionStats(progress) {
  const startTime = progress.startTime ? new Date(progress.startTime) : new Date();
